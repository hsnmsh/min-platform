namespace MinPlatform.Validators.Service.Definitions
{
    ...
    public sealed class CustomDefinition<TProperty> : BaseDefinition
    {
        internal override string DefinitionName => "custom";

        public IDictionary<string, string> FunctionErrors
        {
            get;
            set;
        }
        ...
        internal override ValidationCheckResult CheckValidatorsPerDefinition(object value)
        {
            ...
            foreach (IValidator<TProperty> validator in Validators)
            {
                var validatonResult = validator.IsValid(this, (TProperty)value);

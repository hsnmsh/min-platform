namespace MinPlatform.Validators.Service.Validators
{
    ...
    internal sealed class EndWithValidator : IValidator<string>
    {
        private readonly string stringPart;
        private readonly bool ignoreCase;

        public EndWithValidator(string stringPart)
        {
            this.stringPart = stringPart ?? throw new ArgumentNullException(nameof(stringPart));
        }

        public EndWithValidator(string stringPart, bool ignoreCase) : this(stringPart)
        {
        ...
        public ValidationCheckResult IsValid(BaseDefinition typeDefinition, string value)
        {
            ...
            {
                throw new ValidatorException("invalid definition for End With validator");
            }

            if (!CommonValidators.EndWith(value, stringPart, ignoreCase))
            {

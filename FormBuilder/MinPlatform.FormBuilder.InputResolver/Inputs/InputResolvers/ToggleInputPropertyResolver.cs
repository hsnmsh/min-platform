namespace MinPlatform.FormBuilder.Elements.Inputs.InputResolvers
{
    ...
    internal sealed class ToggleInputPropertyResolver : BaseInputPropertyResolver<TogglePropertyInput, ToggleInputType>
    {
        public ToggleInputPropertyResolver(TogglePropertyInput propertyInputType,
        ...
        public async override Task<ToggleInputType> ResolveInputAsync()
        {
            await SetCommonPropertiesAsync();

            return new ToggleInputType
            {
                InputType = InputType.Toggle.ToString().ToLower(),

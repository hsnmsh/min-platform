...
namespace MinPlatform.Schema.Migrators.Runner.BatchParser
{
    using System;
    ...
    internal class SearchStatus
    {
        ...
        [NotNull, ItemNotNull]
        private readonly Stack<IRangeSearcher> activeRanges;

        [CanBeNull]
        private readonly SpecialTokenInfo foundToken;
        ...
        {
            Debug.Assert(
                activeRanges.Count != 0 || foundToken == null,
                "Operation can only be performed when a range is active and no token has been found");

            var searcher = activeRanges.Peek();
            var result = searcher.FindEndCode(reader);
            if (result == null)
            {
                var reader = this.reader;
                if (context.StripComments && searcher.IsComment)
                {
                ...
                return new SearchStatus(context, reader, activeRanges, null);
            }
            ...
            var nextReader = WriteSql(reader, searcher, result);
            if (nextReader == null)
                return null;

            activeRanges.Pop();
            return new SearchStatus(context, nextReader, activeRanges, null);
        }
        ...
        {
            Debug.Assert(activeRanges.Count == 0, "Operation can only be performed when no range is active");

            var rangeStart = FindRangeStart(reader, context.RangeSearchers);
            var tokenInfo = FindToken(reader, context.SpecialTokenSearchers);
            if (tokenInfo != null)
            {
                if (rangeStart == null || rangeStart.Index > tokenInfo.Index)
                {
                    var reader = WriteSql(this.reader, tokenInfo.Index, tokenInfo.Length);
                    context.OnSpecialToken(new SpecialTokenEventArgs(tokenInfo.Token, tokenInfo.Opaque));
                    if (reader == null)
                        return null;
                    return new SearchStatus(context, reader, activeRanges, tokenInfo);
                }
            }
            else if (rangeStart == null)
            {
                var reader = WriteSql(this.reader);
                if (reader == null)
                    return null;
                return new SearchStatus(context, reader, activeRanges, null);
            }

            return UseNewRange(reader, rangeStart);
        }
        ...
        private SearchStatus UseNewRange([NotNull] ILineReader reader, [NotNull] RangeStart info)
        {
            ...
            activeRanges.Push(info.Searcher);
            return new SearchStatus(context, nextReader, activeRanges, null);
        }
        ...
        private ILineReader WriteSql([NotNull] ILineReader reader)
        {
            ...
            context.OnBatchSql(new SqlBatchCollectorEventArgs(content, true));
        ...
        {
            if (info.Searcher.IsComment && context.StripComments)
        ...
        {
            if (searcher.IsComment && context.StripComments)
            {
                var length = info.Index - reader.Index + searcher.EndCodeLength;
                if (length == reader.Length)
                    context.OnBatchSql(new SqlBatchCollectorEventArgs(string.Empty, true));
                return reader.Advance(length);
            }

            return WriteSql(this.reader, info.Index + searcher.EndCodeLength);
        }
        ...
        private ILineReader WriteSql([NotNull] ILineReader reader, int itemIndex, int skipLength = 0)
        {
            ...
            {
                context.OnBatchSql(new SqlBatchCollectorEventArgs(content, isEndOfLine));
            }

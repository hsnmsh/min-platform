...
namespace MinPlatform.Schema.Migrators.Runner.BatchParser.RangeSearchers
{
    ...
    public class StringWithNoEscape : IRangeSearcher
    {
        ...
        public StringWithNoEscape(string startAndEndCode, bool isComment = false)
        {
            ...
            startCodeRegex = endCodeRegex = new Regex(codePattern, RegexOptions.CultureInvariant | RegexOptions.Compiled);
        }
        ...
        {
            var match = endCodeRegex.Match(reader.Line, reader.Index);
            if (!match.Success)
                return null;
            return match.Index;
        }

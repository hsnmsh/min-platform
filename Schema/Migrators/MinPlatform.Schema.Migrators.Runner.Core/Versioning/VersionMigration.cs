...
namespace MinPlatform.Schema.Migrators.Runner.Versioning
{
    ...
    public class VersionMigration : Migration
    {
        ...
        {
            var builder = Create.Table(versionTableMetaData.TableName)
                .InSchema(versionTableMetaData.SchemaName)
                .WithColumn(versionTableMetaData.ColumnName).AsInt64().NotNullable();

            if (versionTableMetaData.CreateWithPrimaryKey)
            {
                builder.PrimaryKey(versionTableMetaData.UniqueIndexName);
            }
        }

        public override void Down()
        {
            Delete.Table(versionTableMetaData.TableName).InSchema(versionTableMetaData.SchemaName);
        }
    }

...
namespace MinPlatform.Schema.Migrators.Runner.Processors
{
    ...
    using System.Data;
    using System.Linq;
    using MinPlatform.Schema.Migrators.Runner.Announcers;
    ...
    {
        [NotNull] private readonly LoggerManager logger;

        public ConnectionlessProcessor(
            [NotNull] IGeneratorAccessor generatorAccessor,
            LoggerManager logger,
            ProcessorOptions options,
            SelectingProcessorAccessorOptions accessorOptions)
        {
            this.logger = logger;
            var generator = generatorAccessor.Generator;
            DatabaseType = string.IsNullOrEmpty(accessorOptions.ProcessorId) ? generator.GetName() : accessorOptions.ProcessorId;
            Generator = generator;
            Options = options;
...
#pragma warning restore 612
        ...
        {
            logger.Information("Performing DB Operation");
        }
...
#pragma warning restore 618
        ...
        public void Process(CreateDataBaseExpression expression)
        {
            Process(Generator.Generate(expression));
        }

        public void Process(DeleteDataBaseExpression expression)
        {
            Process(Generator.Generate(expression));
        }
    }

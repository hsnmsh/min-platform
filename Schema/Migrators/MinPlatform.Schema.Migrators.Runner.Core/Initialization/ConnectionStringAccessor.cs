...
namespace MinPlatform.Schema.Migrators.Runner.Initialization
{
    ...
    {
        private readonly Lazy<string> lazyData;
        ...
        public ConnectionStringAccessor(
            ...
            SelectingProcessorAccessorOptions processorSelectorOptions,
            IEnumerable<IConnectionStringReader> readers)
        {
            lazyData = new Lazy<string>(
                () =>
                {
                    var connectionStringOrName =
                        string.IsNullOrEmpty(processorOptions.ConnectionString)
                            ? processorSelectorOptions.ProcessorId
        ...
        /// <inheritdoc />
        public string ConnectionString => lazyData.Value;
    }
}

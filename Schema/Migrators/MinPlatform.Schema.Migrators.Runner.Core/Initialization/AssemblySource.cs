...
namespace MinPlatform.Schema.Migrators.Runner.Initialization
{
    ...
    public class AssemblySource : IAssemblySource
    {
        ...
        public AssemblySource(AssemblySourceOptions options, IEnumerable<IAssemblyLoadEngine> loadEngines, IEnumerable<IAssemblySourceItem> sourceItems)
        {
            assemblies = new Lazy<IReadOnlyCollection<Assembly>>(
                () => LoadAssemblies(options, loadEngines.ToList())
        ...
        {
            assemblies = new Lazy<IReadOnlyCollection<Assembly>>(() => assemblyCollectionFunc().Assemblies);
        }

        /// <inheritdoc />
        public IReadOnlyCollection<Assembly> Assemblies => assemblies.Value;

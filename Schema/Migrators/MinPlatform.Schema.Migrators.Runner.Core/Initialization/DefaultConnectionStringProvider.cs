...
namespace MinPlatform.Schema.Migrators.Runner.Initialization
{
    ...
    public class DefaultConnectionStringProvider : IConnectionStringProvider
    {
        [CanBeNull]
        [ItemNotNull]
        private readonly IReadOnlyCollection<IConnectionStringReader> accessors;
            ...
            string database)
        {
            if (_connectionString == null)
            {
                lock (_syncRoot)
                {
                    if (_connectionString == null)
                    {
                        var accessors = this.accessors ?? CreateAccessors(assemblyLocation, announcer, configPath).ToList();
